...
while (true) {
    ...
    $line = readline("Entrez votre commande (aide, lister, détailler, créer, supprimer, quitter) : ");
    ...
    if (preg_match("/^détailler (.*)$/", $line, $matches)) {
        // Exemple : detail 1
    ...
    if (preg_match("/^créer (.*), (.*), (.*)$/", $line, $matches)) {
    ...
    if (preg_match("/^supprimer (.*)$/", $line, $matches)) {
        // Exemple : delete 1
